...
export class ChatRoomPage implements OnDestroy,OnInit{
  interval:any;
  sendCon:string = "";
  sendBtn: any;
  kind:string = "";
  ...
  ionViewDidEnter(){
    this.jmGroupId = this.navParams.get('jmGroupId');
    this.title = this.navParams.get('title');
    this.resetUnreadMessageCount();//重置消息未读数;
    this.userId = window.localStorage.getItem('userId');
    this.getHistoryMessages();//获取历史消息;
    this.events.subscribe('chat:chatRoom', (msg) => {
      if(this.jmGroupId == msg.target.id && msg.target.type == 'group'){
        //this.push()
        switch (msg.type){
          case 'text':
  ...
  sendMessage(){
    ...
    this.chatService.sendTextMessage({ type: 'group', groupId: this.jmGroupId, text: this.sendCon},(res) =>{
...
//重置未读消息
  resetUnreadMessageCount(){
    ...
    this.chatService.resetUnreadMessageCount(this.jmGroupId);
  }

  //获取历史消息
  getHistoryMessages(){
    console.log(JSON.stringify({ type: 'group', groupId: this.jmGroupId, from: 0, limit: 10 }),"获取历史消息");
    this.chatService.getHistoryMessages({ type: 'group', groupId: this.jmGroupId, from: 0, limit: 10 },(res) =>{
      console.log(JSON.stringify(res),"------获取历史消息212121");
      this.getCurrentUser(()=>{
  ...
  onRecord(params) {
    ...
    //通过极光发送消息
    this.chatService.sendVoiceMessage({ type: 'group', groupId: this.jmGroupId, path: params.filePath, extras: {second: params.second,minute:params.minute}},
      (res)=>{
  ...
  getPicture(){
    this.albumCamera.getPicture((res)=>{
      for(let i =0;i<res.length;i++){
        ...
        this.chatService.sendImageMessage({type: 'group', groupId: this.jmGroupId, path: res[i]},(res)=>{
  ...
  getCamera(){
    this.albumCamera.getCamera((res)=>{
      if(res){
        ...
        this.chatService.sendImageMessage({type: 'group', groupId: this.jmGroupId, path: res},(res)=>{
          // do something
        },(err)=>{
          //console.log(err);
        });
